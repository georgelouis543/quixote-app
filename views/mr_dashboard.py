...
@st.cache_data
def mr_event_analytics_aggregator(input_df):
    try:
        input_df = input_df
        ...
        for unique_arg, group in grouped:
            ...
            out_dict1["Company_ID"] = dict_unique_arg["companyId"]
...
def clicks_aggregator(input_df):
    ...
    for url in filtered_clicked_urls_list:
        out_url_dict = {}
        filtered_url_df = df[df["url"] == url]
        out_url_dict["url"] = url
...
try:
    if uploaded_file is not None:
        ...
        event_analytics_df = mr_event_analytics_aggregator(df)
